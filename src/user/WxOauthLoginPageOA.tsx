...
import { LoginParam, OAuthInfo } from './AuthData';
import { DEBUG, Host } from '@/config';
import { WebAppHelper } from './WebAppHelper';
import { login } from './WxOauthNotifyOA';
...
const WxOauthLoginPageOA: React.FC<LoginParam> = (props: LoginParam) => {
    const [status, setStatus] = useState<string>()
    
    //const query: CorpParams =  f7.utils.parseUrlQuery(props.from);
    const appId = WebAppHelper.getCorpParams()?.appId || props.appId 

    const {from, needUserInfo, owner, authStorageType} = props


    const directlyRedirectOAuth = (needUserInfo: boolean, appId?: string,  from?: string) => {
        if(!appId){
            console.log("no appId, please set appId in query parameters")
    ...
    const getOauthInfo = (openId?: string) => {
        ...
        getWithouAuth( '/api/wx/oa/oauth/info', {appId: appId, owner: props.owner, openId: openId}) //另：可以指定通知host
       //前端负责state，拼接跳转url， state校验，然后直接进行跳转，不再向后端请求
            .then(function (res) {
                ...
                if(openId && !oauthInfo.needUseInfo){ //若已有openId，且无需获取userInfo，直接打开页面
                    ...
                    WxGuestAuthHelper.onAuthenticated({ appId: appId, openId1: openId, hasUserInfo: true}, authStorageType||StorageType.BothStorage)
                    if(from) f7.views.main.router.navigate(from) //window.location.href = from //props.f7router.navigate(from) 
                }else{
    ...
    useEffect(()=> {
        if(needUserInfo === undefined){ //没有明确指定是否获取userInfo，通常用于newsDetail页面
            if(!owner){
                ...
                directlyRedirectOAuth(true, appId, from) //没有owner情况，都认为需要获取userinfo
            }else{
            ...
            if(silentLogin){//明确要求静默登录
                ...
                const openId = WxGuestAuthHelper.getAuthBean()?.openId1 || WxAuthHelper.getAuthBean()?.openId1
                if(!openId){
                    //无openId
                    directlyRedirectOAuth(needUserInfo, appId, from)
                }else{
                    ...
                    login(openId, () => console.log("successful to login"),
                        ...
                        (msg) => f7.dialog.alert("出错了：试试关闭窗口，重新打开"+msg),authStorageType||StorageType.BothStorage
                    )
                }
            }else{
